...
Filter couchDB output to produce just the output below. This implies to remove the \verb|password| key and value always.

Auth reponse is true:
\begin{lstlisting}
{
  "total_rows": 2,
  "offset": 0,
...
Auth response is false: example result: \verb|403 Forbidden|
...
Mapping to CouchDB: \verb|GET /nodes/_design/nodes/_list/names/byId|
\begin{lstlisting}
{
...
\subsubsection{GET /nodes/\_description}
\label{protocoldef:microservicesapi:database:getnodes/_descriptions}
Return just an JSON array of all nodenames and their description, sorted as a list. Used where descriptive selections have to be done on the website.
For all roles example result:
Mapping to CouchDB: \verb|GET /nodes/_design/nodes/_list/descriptions/descriptions|
\begin{lstlisting}
{
  [
    {
...
Mapping to CouchDB: \verb|GET /rubrics/_design/rubrics/_list/names/byId|
...
Mapping to CouchDB: \verb|GET /rubrics/_design/rubrics/_list/descriptions/descriptions|
...
\label{tab:protocoldef:permissionmatrix:usersdb}
  \begin{tabular}{|l|c|c|c|c|c|} \hline
    ...
    \verb|user.read|        & all   & all     & none   & none  & none \\ \hline
    \verb|user.create|      & all   & all     & none      & none  & none      \\ \hline
    \verb|user.update|      & all   & all     & limited & none  & limited \\ \hline
    \verb|user.delete|      & all   & all     & limited & none  & limited \\ \hline
    \verb|user.change_role| & all   & none    & none      & none  & none      \\ \hline
  \end{tabular}
  \caption{Role's permissions for users database}
\end{table}
\textbf{limited}: \verb|_id, _rev, roles, email, created_on, created_by, changed_on, changed_by|
...
\label{tab:protocoldef:permissionmatrix:transmittersdb}
  \begin{tabular}{|l|c|c|c|c|c|} \hline
    ...
    \verb|transmitter.read|        & all   & all     & limited & none     & if\_owner \\ \hline
    \verb|transmitter.create|      & all   & all     & none      & none     & none      \\ \hline
    \verb|transmitter.update|      & all   & all     & limited & none     & if\_owner \\ \hline
    \verb|transmitter.delete|      & all   & all     & limited & none     & if\_owner \\ \hline \hline
...
\label{tab:protocoldef:permissionmatrix:subscribersdb}
  \begin{tabular}{|l|c|c|c|c|c|} \hline
    ...
    \verb|subscriber.update|      & all   & all     & limited & none  & limited \\ \hline
    \verb|subscriber.delete|      & all   & all     & limited & none  & limited \\ \hline
...
\label{tab:protocoldef:permissionmatrix:nodesdb}
  \begin{tabular}{|l|c|c|c|c|c|} \hline
    ...
    \verb|node.read\_full|   & all   & all     & none  & none    & none  \\ \hline
